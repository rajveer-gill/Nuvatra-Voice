...
Test location for Nuvatra Voice. Full Pro features: staff forwarding, customer memory, call logging, analytics.
...
- Reservations and messages (stored in dashboard)
- Appointment review: copy-paste block, Accept (sends confirmation SMS), Reject (releases slot and sends "when else available?" SMS)
- Slot memory so the AI does not double-book; suggests another time or staff if slot is taken
